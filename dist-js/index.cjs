...
async function getPowerState() {
    return await core.invoke('plugin:power-monitor|get_power_state');
}
/**
...
async function onPowerSourceChanged(handler) {
    return await event.listen('power-monitor://power-source-changed', (event) => {
...
async function onBatteryChanged(handler) {
    return await event.listen('power-monitor://battery-changed', (event) => {
        handler(event.payload);
    });
}
/**
...
async function onSuspend(handler) {
    return await event.listen('power-monitor://suspend', () => {
        handler();
    });
}
/**
...
async function onResume(handler) {
    return await event.listen('power-monitor://resume', () => {
        handler();
    });
}

exports.getPowerState = getPowerState;
exports.onBatteryChanged = onBatteryChanged;
exports.onPowerSourceChanged = onPowerSourceChanged;
